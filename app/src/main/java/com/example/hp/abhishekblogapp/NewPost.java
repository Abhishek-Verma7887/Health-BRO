...
public class NewPost extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        newPostTopic = findViewById(R.id.new_topic_add_post);
...
//                    final String timestamp = FieldValue.serverTimestamp().toString();
                    ...
                    filePath.putFile(main_uri,metadata).addOnCompleteListener(new OnCompleteListener<UploadTask.TaskSnapshot>() {
                        @Override
                        public void onComplete(@NonNull final Task<UploadTask.TaskSnapshot> task) {
                            if(task.isSuccessful()){
                              ...
                              filePath.getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
                                ...
                                final StorageReference newfilepath =storageReference.child("/post_images/thumbs").child(randomName+".jpg");
                                StorageMetadata metadata2 = new StorageMetadata.Builder().setContentType("image/jpg").build();
                                UploadTask thumbImage = newfilepath.putBytes(thumbBitmap,metadata2);
                                thumbImage.addOnSuccessListener(new OnSuccessListener<UploadTask.TaskSnapshot>() {
                                    ...
                                    public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {
                                        if(task.isSuccessful()){
                                            ...
                                            final String[] thumbUri = new String[1];
                                            newfilepath.getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
                                                @Override
                                                public void onSuccess(Uri uri2) {
                                                    thumbUri[0] = uri2.toString();
                                                }
                                            });
                                            Toast.makeText(NewPost.this, thumbUri[0]+"20", Toast.LENGTH_LONG).show();
                                            Map<String,Object> postMap = new HashMap<>();
                                            postMap.put("image_url", downloadUrl[0]);
                                            postMap.put("desc",postText);
                                            postMap.put("topicNameName",postTopic);
                                            postMap.put("user_id",user_id);
                                            postMap.put("image_thumb", thumbUri[0]);
                                            ...
                                            firebaseFirestore.collection("Posts").add(postMap).addOnSuccessListener(new OnSuccessListener<DocumentReference>() {
                                                ...
                                                public void onSuccess(DocumentReference documentReference) {
                                                    documentReference.update("image_thumb",thumbUri[0]);
                                                    Toast.makeText(NewPost.this, thumbUri[0]+"30", Toast.LENGTH_LONG).show();
                                                }
                                            })
